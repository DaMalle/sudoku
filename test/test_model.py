...
class TestSolutionGenerator(unittest.TestCase):
    ...
    def test_format(self):
        ...
        self.assertEqual(len(self.solution), 81)
        self.assertTrue(all(1 <= x <= 9 for x in self.solution))
    ...
    def test_is_valid_row(self):
        ...
        for i in range(9):
            with self.subTest(i=i):
                ...
                self.assertEqual(set(self.solution[i*9:i*9+9]), set(range(1, 10, 1)))
    ...
    def test_is_valid_column(self):
        ...
        for i in range(9):
            with self.subTest(i=i):
                ...
                self.assertEqual(set(self.solution[i*9+j] for j in range(9)), set(range(1, 10, 1)))
    ...
    def test_is_valid_box(self):
        ...
        for i in range(9):
            with self.subTest(i=i):
                ...
                self.assertEqual(
                    set(self.solution[((i // 3) * 3 + k) * 9 + ((i % 3) * 3 + j)]
                        for k in range(3)
                        for j in range(3)),
                    set(range(1, 10, 1))
                )
